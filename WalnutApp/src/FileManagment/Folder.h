...
namespace FastFileExplorer
{
	...
	public:
		Folder(std::string folderName, std::string folderPath, bool computeSize = true)
			: m_FolderName(folderName), m_FolderPath(folderPath), m_ComputeSize(computeSize)
		{
		}
		...
		std::string GetSize(bool ignoreCache = false)
		{
			...
			std::cout << "no cached folder size for folder " << m_FolderName << std::endl;
			...
			m_Size = std::to_string(totalSize) + "b";
